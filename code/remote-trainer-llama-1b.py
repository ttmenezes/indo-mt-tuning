...
def generate_llama(
    ...
    input_ids = chat_template.to(model.device)
...
# generations = generate_llama(model, prompts)
...
def formatting_prompts_func(example):
    ...
    for i in range(len(example['text'])):
        messages = [
            {
                "role": "user",
                "content": f"Translate from Javanese to Indonesian: {example['text'][i]}"
            },
            {
                "role": "assistant",
                "content": example['target'][i]
            }
        ]
        # Let the tokenizer handle the chat template formatting
...
trainer.model.save_pretrained(save_directory='./outputs/llama-1b-javanese-sft-adapter')
...
MODEL_PATH = './outputs/llama-1b-javanese-sft-adapter'
...
tokenizer.pad_token = tokenizer.eos_token  # Set padding token to EOS token
tokenizer.padding_side = "right"  # Pad on the right side
loaded_sft_model.load_adapter("./outputs/llama-1b-javanese-sft-adapter")
...
generations = generate_llama(loaded_sft_model, prompts)
